...
class ApprovedAppointmentTab extends StatefulWidget {
  const ApprovedAppointmentTab({super.key});

  @override
  State<ApprovedAppointmentTab> createState() => _ApprovedAppointmentTabState();
}
...
class _ApprovedAppointmentTabState extends State<ApprovedAppointmentTab> {
  ...
  Widget build(BuildContext context) {
    ...
    return appointmentsProvider.approvedAppointments.isEmpty
        ? Center(
            child: Text(getTranslated(context, LangConst.noDataFound).toString()),
          )
        : ListView.separated(
            shrinkWrap: true,
            itemCount: appointmentsProvider.approvedAppointments.length,
            padding: EdgeInsets.all(
              Amount.screenMargin,
            ),
            separatorBuilder: (context, index) => HeightBox(10),
            itemBuilder: (context, index) {
              AppointmentObject data = appointmentsProvider.approvedAppointments[index];
              return Container(
